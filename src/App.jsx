...
const cuisines = [
  { name:"Indian",        emoji:"🍛", tag:"Spicy & Aromatic",  grad:["#ff6b35","#f7c59f"], text:"#7a2000" },
  { name:"Italian",       emoji:"🍝", tag:"Rich & Comforting",  grad:["#2d6a4f","#95d5b2"], text:"#0d3320" },
  { name:"Chinese",       emoji:"🥡", tag:"Bold & Savory",      grad:["#c1121f","#ffb703"], text:"#5a0000" },
  { name:"Mexican",       emoji:"🌮", tag:"Vibrant & Zesty",    grad:["#e9c46a","#f4a261"], text:"#6b3800" },
  { name:"Mediterranean", emoji:"🫙", tag:"Fresh & Light",      grad:["#457b9d","#a8dadc"], text:"#0d3347" },
  { name:"Japanese",      emoji:"🍱", tag:"Delicate & Umami",   grad:["#7b2d8b","#c77dff"], text:"#35004a" },
  { name:"Thai",          emoji:"🍜", tag:"Sweet & Spicy",      grad:["#06b6d4","#67e8f9"], text:"#013d47" },
  { name:"American",      emoji:"🍔", tag:"Hearty & Bold",      grad:["#ef233c","#8d99ae"], text:"#5a0010" },
];

const dietTabs = [
  { key:"Veg",     label:"Vegetarian",     icon:"🥦", color:"#2d6a4f", bg:"#d8f3dc" },
  { key:"Non-Veg", label:"Non-Vegetarian", icon:"🍗", color:"#9b2226", bg:"#fde8e8" },
  { key:"Vegan",   label:"Vegan",          icon:"🌱", color:"#386641", bg:"#ddf3dd" },
];
...
export default function TasteMania() {
  ...
  const fetchRecipes = async (c, d) => {
    ...
    try {
      ...
      setRecipes(JSON.parse(data.content[0].text.replace(/```json|```/gi,"").trim()));
    } catch {
      setError("⚠️ API key needed. Set ANTHROPIC_API_KEY in Netlify → Site Settings → Environment Variables.");
      setRecipes([
  ...
  const fetchDetail = async (r) => {
    ...
    try {
      ...
      const data = await res.json();
      setDetail(JSON.parse(data.content[0].text.replace(/```json|```/gi,"").trim()));
    } catch {
      setDetail({ ingredients:["Set ANTHROPIC_API_KEY in Netlify to load live recipes."], steps:["Once API key is configured, full instructions will appear here."], tip:"" });
    }
    setDL(false);
  };
  ...
  const Label = ({children}) => (
    <p style={{ color:G.warm, fontWeight:700, fontSize:11, letterSpacing:2.5, textTransform:"uppercase", marginBottom:10 }}>{children}</p>
  );
  ...
  return (
    <div style={{ background:G.cream, minHeight:"100vh", fontFamily:"'Segoe UI',system-ui,sans-serif" }}>
      ...
      {step==="home" && <>
        ...
        {/* HERO — full gradient, no external image */}
        <div style={{ position:"relative", minHeight:500, background:"linear-gradient(135deg,#1a1a1a 0%,#2d3a2e 40%,#3b1f0a 100%)", display:"flex", alignItems:"center", overflow:"hidden" }}>
          ...
          {["#ff6b3520","#c9882b18","#2d6a4f22"].map((c,i)=>(
            <div key={i} style={{ position:"absolute", width:380+i*80, height:380+i*80,
              borderRadius:"50%", background:c,
              top: i===0?-80: i===1?120:-60,
              right:i===0?-60: i===1?200:-20,
              pointerEvents:"none" }} />
          ))}
          <div style={{ position:"relative", zIndex:2, padding:"3.5rem 6%", maxWidth:600 }}>
            ...
            <button onClick={scrollToCuisines}
              style={{ background:G.green, color:"#fff", padding:"15px 36px", border:"none", borderRadius:6, fontWeight:700, fontSize:16, cursor:"pointer", letterSpacing:0.5 }}>
              Browse Cuisines ↓
            </button>
          </div>
          {/* big emoji float */}
          <div style={{ position:"absolute", right:"5%", top:"50%", transform:"translateY(-50%)", fontSize:140, opacity:0.18, pointerEvents:"none", userSelect:"none" }}>🍽️</div>
        </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))", gap:20 }}>
            {cuisines.map((c)=>(
              <div key={c.name} className="pop" onClick={()=>goTo(c)}
                style={{ height:210, borderRadius:16, overflow:"hidden", cursor:"pointer", transition:"all 0.2s", boxShadow:"0 4px 18px rgba(0,0,0,0.10)",
                  background:`linear-gradient(135deg,${c.grad[0]},${c.grad[1]})`,
                  display:"flex", flexDirection:"column", justifyContent:"flex-end", padding:"20px 20px" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))", gap:24 }}>
              {TRENDING.map((t)=>(
                <div key={t.name} className="lift"
                  ...
                  <div style={{ height:190, background:`linear-gradient(135deg,${t.grad[0]},${t.grad[1]})`,
      ...
      {step==="diet" && (
        <div style={{ maxWidth:1060, margin:"0 auto", padding:"2.5rem 1.5rem" }}>
          ...
          {/* cuisine hero banner */}
          {cuisineObj && (
            <div style={{ borderRadius:14, padding:"2rem 2.5rem", marginBottom:32, display:"flex", alignItems:"center", gap:24,
              background:`linear-gradient(135deg,${cuisineObj.grad[0]},${cuisineObj.grad[1]})` }}>
          ...
          <div style={{ display:"flex", gap:12, flexWrap:"wrap", marginBottom:32 }}>
            {dietTabs.map(d=>{
              ...
              return (
                <button key={d.key} className="dtab" onClick={()=>changeDiet(d.key)}
                  style={{ padding:"11px 24px", borderRadius:6, border:`2px solid ${d.color}`,
                    background:active?d.color:"#fff", color:active?"#fff":d.color,
          ...
          {loading ? (
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(290px,1fr))", gap:24 }}>
              {recipes.map((r,i)=>{
                const [accent, light] = CARD_COLORS[i%CARD_COLORS.length];
                return (
                  <div key={i} className="lift" onClick={()=>openRecipe(r)}
                    ...
                    <div style={{ height:160, background:`linear-gradient(135deg,${accent}cc,${accent}88)`,
                      display:"flex", alignItems:"center", justifyContent:"center",
                      fontSize:80, position:"relative" }}>
                      <span style={{ filter:"drop-shadow(0 4px 10px rgba(0,0,0,0.2))" }}>
                        {cuisineObj?.emoji||"🍽️"}
                      </span>
      ...
      {step==="detail" && recipe && (
        <div style={{ maxWidth:820, margin:"0 auto", padding:"2.5rem 1.5rem" }}>
          ...
          <div style={{ background:"#fff", borderRadius:16, overflow:"hidden", boxShadow:"0 4px 24px rgba(0,0,0,0.09)" }}>
            {/* hero band */}
            <div style={{ height:260, background:`linear-gradient(135deg,${cuisineObj?.grad[0]||"#2d6a4f"},${cuisineObj?.grad[1]||"#95d5b2"})`,
